package yarangi.graphics.quadraturin.samples.nn;
...
import yarangi.ai.nn.numeric.LogisticAF;
...
//		net.addLayer(new CompleteNeuronLayer(3, new TanHAF(), 0.01));
		net.addLayer(new CompleteNeuronLayer(10, new TanHAF(), 1));
		net.addLayer(new CompleteNeuronLayer(9, new TanHAF(), 1));
...
//			double y = Math.random() * (maxx-minx) + minx;
			...
			tys[idx] = new double []{Math.sin(x) + Math.random() * noiseDB - noiseDB/2};
...
//			learningRate *= 1.001;
			double [] nx = new double[(int)((maxx-minx+4)/0.05)+1];
			double [] ny = new double[nx.length];
			      
			int idx = 0;
			for(double x = minx-2; x < maxx+2; x += 0.05)
			{
				nx[idx] = x;
